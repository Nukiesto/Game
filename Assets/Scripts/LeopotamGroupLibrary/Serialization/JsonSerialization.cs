...
namespace LeopotamGroup.Serialization
{
    ...
    public class JsonSerialization
    {
        ...
        private void SerializeMember(object obj)
        {
            if (obj == null)
            {
                return;
        ...
        {
            private static bool IsWordBreak(int c)
            {
            ...
            private readonly Stack<ArrayList> _arrayPool = new Stack<ArrayList>(16);

            private readonly char[] _hexBuf = new char[4];

            private readonly StringBuilder _stringBuf = new StringBuilder(128);
            ...
            private ArrayList GetArrayItem()
            {
            ...
            private void RecycleArrayItem(ArrayList item)
            {
                if (item != null)
                {
                    item.Clear();
            ...
            private object ParseByToken(JsonToken token)
            {
                switch (token)
                {
        ...
        private sealed class TypesCache
        {
            ...
            private readonly Dictionary<Type, TypeDesc> _types = new Dictionary<Type, TypeDesc>(32);
