...
namespace LeopotamGroup.Threading
{
    ...
    public abstract class BackgroundWorkerBase<Base, T> : MonoBehaviourService<Base> where Base : class
    {
        ...
        private bool _dontSleepAfterItemProcess;

        private readonly object _inSyncObj = new object();

        private readonly object _outSyncObj = new object();

        private readonly List<T> _inQueue = new List<T>(64);

        private readonly List<T> _outQueue = new List<T>(64);
        ...
        private void OnBackgroundThreadProc()
        {
            lock (_inSyncObj)
            {
