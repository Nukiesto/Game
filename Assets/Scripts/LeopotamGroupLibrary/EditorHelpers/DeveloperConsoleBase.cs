...
namespace LeopotamGroup.EditorHelpers
{
    ...
    internal abstract class DeveloperConsoleBase<T> : MonoBehaviourService<T> where T : class
    {
        ...
        private const string MarkupSchema = "LeopotamGroup/EditorHelpers/DevConsole";

        private const string MarkupTheme = "LeopotamGroup/EditorHelpers/DevConsoleTheme";

        private const string MarkupScrollViewName = "logScroll";

        private const string MarkupLogTextName = "logText";

        private const string MarkupInputName = "input";

        private const float CloseButtonWidth = 128f;

        private ScriptVm _vm;

        private MarkupContainer _markup;

        private InputField _inputField;

        private Text _logText;

        private RectTransform _logScroll;

        private string[] _logLines;

        private int _linesCount;

        private int _onDevConsoleId;
        ...
        private void OnInputEnd(UiInputEndActionData arg)
        {
        ...
        private void OnClose(UiClickActionData arg)
        {
            Show(false);
        }
