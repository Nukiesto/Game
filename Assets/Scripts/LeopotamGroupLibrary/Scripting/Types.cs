...
{
    /// <summary>
    /// Тип переменной ScriptVm.
    /// </summary>
    internal enum ScriptVarType
    {
        Undefined = 0,
    ...
    {
        private readonly Dictionary<string, FunctionDesc> _functions = new Dictionary<string, FunctionDesc>(16);

        private readonly Dictionary<string, ScriptVar> _vars = new Dictionary<string, ScriptVar>(128);

        private readonly Dictionary<string, HostFunction> _hostFuncs = new Dictionary<string, HostFunction>(128);
    ...
    internal delegate ScriptVar HostFunction(ScriptVm vm);
}