...
{
    /// <summary>
    /// Ѕазовый класс диспетчера скриптов. ќбертка вокруг экземпл€ра ScriptVm, предоставл€ет из него api.
    /// </summary>
    internal abstract class ScriptManagerBase<T> : MonoBehaviourService<T> where T : class
    {
        private struct TimeoutPair
        {
            public float Time;
        ...
        private readonly ScriptVm _vm = new ScriptVm();

        private readonly List<TimeoutPair> _timeoutListeners = new List<TimeoutPair>(16);
        ...
        private ScriptVar ApiCallWithDelay(ScriptVm vm)
        {
        ...
        private ScriptVar ApiDebug(ScriptVm vm)
        {
