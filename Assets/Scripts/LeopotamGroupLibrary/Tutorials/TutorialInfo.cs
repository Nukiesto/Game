...
namespace LeopotamGroup.Tutorials
{
    ...
    public sealed class TutorialInfo : MonoBehaviour
    {
        ...
        private void OnTutorialUpdated()
        {
            var isProcessed = false;
            var result = false;

            if (HideChildrenOnMask != 0 && Service<TutorialManager>.Get().ValidateMask(HideChildrenOnMask))
            {
                isProcessed = true;
            }
            if (!isProcessed && ShowChildrenOnMask != 0 && Service<TutorialManager>.Get().ValidateMask(ShowChildrenOnMask))
            {
                isProcessed = true;
                result = true;
            }
